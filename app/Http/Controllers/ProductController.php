...
class ProductController extends Controller
{
    ...
    public function index(Product $product) {
        ...
        $this->data['dataProducts'] = $product->where('created_by', $this->user->id)->get();
        return view('manage.product.index', $this->data);
    }
    //หน้าสร้างสินค้า
    public function create(TypeProduct $typeProduct) {

        $this->data['dataTypeProducts'] = $typeProduct->where('created_by', $this->user->id)->get();
    ...
    public function store(Request $req, Product $product, ProductAttachment $attachment) {
        ...
        $inputs = $req->only('type_product_id', 'fabric_type', 'size', 'name', 'amount', 'detail', 'price');
